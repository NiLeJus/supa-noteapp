...
class NotepadAppSupabaseUser extends BaseAuthUser {
  NotepadAppSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> notepadAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = NotepadAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
